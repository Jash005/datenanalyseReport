...
-->
**=> nochmal anschaun**
Wir extrahieren die Uhrzeit und den Wochentag aus dem `crash_date`, weil in der spalte `crash_hour` einem bestimmten Schema folgt und bevor das überstezt und transformiert wird ist es leichter es aus dem `crash_date` heruaszuextrahieren (`das muss eh beaneberitet werden`). Somit würde die Spalten `crash_hour`, `crash_day_of_week` und `crash_month` wegfallen
...
Ein neues Attribut `season` fasst die Monate zusammen und ordent diese Jahreszeiten hinzu. Um auswertungen zu machen, die von Winter oder Sommer abhängig sind.

Über ein Simples attribut `has_injury` können wir die Unfallursache noch mit der Angabe ob es dabei Verletzte gab, also die Schwere des Unfalls bewerten.
...
In der Spalte `intersection_related_i` sind boolsche Werte enthalten, aber als `Y` und `N` in `character` formatiert. Hier sollten diese in `TRUE` und `FALSE` überführt werden.
...
Nachdem die zeitlichen Aspekte untersucht wurden, richten wir den Fokus nun auf die infrastrukturellen und situativen Rahmenbedingungen der Unfälle. Konkret analysieren wir, ob die Art der Straße (Kreuzung, geteilte Fahrbahn, usw.), der Unfalltyp (Abbiegen, Auffahren, usw.) und die vorhandene Verkehrsregelung (ampel, Stop-Schild, keine Regelung) einen Einfluss auf die Unfallschwere haben, Das Ziel ist zu verstehen, welche Verkehrssituationen besonders risikoreich sind und ob bestimmte Infrastrukturtypen systematisch mit höheren Verletzungsraten eingehen.
...
In den Top 10 Unfalltypen nach absoluter Häufigkeit sind alle drei für unsere Hypothese relevanten Typen ("Head On", "Angle", "Turning") enthalten. Deswegen reichen die Top 10 in der folgenden Heatmap.
...
Deutlich bestätigt wird die Hypothese zu Frontalzusammenstößen. Head On Unfälle auf Two Way Roads zeigen mit 40,8% eine deutlich höhere Verletzungsrate als auf Divided Roads (39,2%). Dies unterstreicht die Schutzwirkung baulicher Mitteltrennungen gegen die gefährlichen Kollisionsarten.

**weitere Erkenntnisse:** Das auffälligste Muster der Heatmap ist die extrem hohe Verletzungsrate bei Pedestrian (89-93%) und Pedalcyclist (70-78%) über alle Straßentypen hinweg, also Fußgänger und Fahrradfahrer. Diese Unfalltypen enden nahezu immer mit Verletzten, unabhängig von dem Straßentypen. Im Gegensatz dazu zeigen klassische Auto gegen Auto Unfälle (Rear End) (13-18%) oder Unfall gegen einem Parkenden Auto (5-18%) deutlich niedrigere Raten. 
Dies wirft die Frage auf, welche Verkehrsregelungen (Ampeln, Vorfahrtsregeln) diese Unfälle möglicherweise hätten verhindern können. 
Konkret: Wie viele Unfälle passieren bei den verschiedenen Verkehrsregelungen, um dann zu sehen, welche am besten vor solchen Unfällen schützen könnten?
